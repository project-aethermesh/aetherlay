...
func main() {
	...
	if !appConfig.StandaloneHealthChecks {
		if appConfig.HealthCheckInterval > 0 {
			...
			defer cancel()
			checker = health.NewChecker(cfg, valkeyClient, time.Duration(appConfig.HealthCheckInterval)*time.Second, time.Duration(appConfig.EphemeralChecksInterval)*time.Second, appConfig.EphemeralChecksHealthyThreshold, appConfig.HealthCheckSyncStatus, appConfig.HealthCheckConcurrency, appConfig.EphemeralChecksEnabled)
